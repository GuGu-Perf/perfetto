...
export class TrackTreeView implements m.ClassComponent<TrackTreeViewAttrs> {
  ...
  private drawCanvas(
    ...
    const {tracksOnCanvas} = renderTimelineCanvas({
      ctx,
      size,
      timelineRect,
      timescale,
      ...
      colors,
      renderer,
      tickOrigin: this.trace.timeline.getTimeAxisOrigin(),
      perfStatsEnabled: this.perfStatsEnabled,
      trackPerfStats: this.trackPerfStats,
      areaDrag: this.areaDrag,
...
export class InProgressAreaSelection {
...
export class InProgressHandleDrag {
  currentTime: HighPrecisionTime;
