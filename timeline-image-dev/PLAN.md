...
**(c) MCP——v9.33 二次修正：定性为"调用方适配器"，perfetto 零改动**。用户价值链推演定案：真正的场景是**第三方 Agent 后台批量分析 trace 出图进报告**，而非 UI 内 1:1 聊天——后者用户本人正看着实时时间线，`show-timeline`（既有 MCP 工具）跳转到 jank 处比在聊天框贴 PNG 更直接；且核验 `gemini_client.ts` 证实模型经 `functionResponse` 只收文本，看不到图（只能读 warnings/trackBoxes metadata）。故插件内 `render-timeline-image` 工具两头价值皆弱，**砍**。

正确形态：进程外入口自始至终只有 postMessage 一个（(a)），**MCP server 只是它的又一个消费者**（与 postmessage-demo.mjs、渲染服务样板同列 §8.5 调用方基建）：`tools/mcp-server.mjs`——stdio MCP server，常驻 headless Chromium 跑 UI，宿主 Agent 的工具调用翻译为 postMessage（完整复用挂起/队列/PNG 回传协议与 demo 已固化的制度：空 TP daemon、150s 硬超时）。工具面 `load_trace` / `list_tracks` / `render_timeline_image`（zod/JSON Schema 从 `TimelineImageOptions` 生成）。不进上游、不动 ui/package.json、零 perfetto diff。

~~v9.32 方案（插件内 ToolRegistry 增量注册）~~：已否决，理由如上；但其架构核验结论保留（官方 MCP=UI 进程内 ToolRegistry + 内置 Gemini 消费端，无独立 server 进程、外部不直达——这本身即"必须自建调用方适配器"的论据）。
...
| 15 | 推翻 ADR-14 的 MCP 半边：插件内工具砍掉；MCP 定性为**调用方适配器**（tools/mcp-server.mjs，postMessage 的又一消费者），perfetto 零改动 | 用户价值链推演：真场景=第三方 Agent 后台批量出图进报告（进程外），非 UI 内 1:1 聊天（人看实时时间线 + 既有 show-timeline 跳转即可）；核验 gemini_client.ts 证实模型经 functionResponse 只收文本、看不到图——插件内工具两头价值皆弱。"无中生有"禁令约束的是 perfetto 仓库内架构；调用方自建 MCP 适配走官方公开 embedding 协议，与 demo/渲染服务同类 | v9.33 |
...
| v9.43 | 08-24 | **loadTrace 耗时调研 + 协议真 bug 修复（探针驱动）**。用户问"能否只加载 CPU 组/线程 track 提速"——源码核验：TP ingest 整包解析（track 是 ingest 后 SQL 表的视图，无按 track 类型选择性加载机制；cropTrackEvents=range-of-interest 语义错位、tokenizeOnly 仅预览），该杠杆不存在。写探针实测分解（58MB example_android_trace）：native 端到端 9.5s（boot 6.5 + ingest ~2.9 + 渲染 130ms）、wasm 9.8s（boot 5.1 + ~4.4 + 277ms）——瓶颈是浏览器启动而非 ingest，native 仅快 ~35%。**探针连抓两个我方 bug**：①探针自身（PONG 是 PING 的回复非广播，首版漏发 PING 致两轮"超时"假象——曾误报 wasm>10min，已当众撤回）；②**协议真 bug**：render/listTracks 挂起条件只等 trace 对象存在（加载早期即建）不等 isLoadingTrace 结束（插件 onTraceLoad 末尾才注册渲染器），大 trace 背靠背投递必现 "no renderer" 错——违反文档承诺。已修（两分支挂起条件加 isLoadingTrace），双引擎 58MB 实测背靠背成功。教训：性能结论必须先修好测量工具；"文档承诺"要用最慢路径验证而非最快路径 |
| v9.42 | 08-24 | **第五轮 review（未覆盖维度：安全/构建注册/locale/资源面）：安全真发现修复**。**漏洞**：renderTimelineImage（外传整图 PNG）与 listTracks（外传 track/进程/线程名）未过 isTrustedOrigin 信任门——上游开 trace 有门（:345），二者对任意持有窗口引用者无条件放行，构成上游从未有过的数据外泄通道（最开放的 scrollToTimeRange 仅操纵视图不外传）。**修复**：两分支加与开 trace 同一信任门（localhost 恒信、其他 origin 经 trace 上传确认的 always-trust 授予），不受信忽略+console.warn；自托管批量流（localhost）零影响——postmessage-demo 实测 7.5s 出图如常。回归用例：伪造 evil.example MessageEvent 断言零回包。文档两份补信任模型说明。其余维度结论：gn 构建注册无需（ui/BUILD=build.mjs 垫片，vite 自动发现）；渲染路径 locale API 零调用（timecode/duration 格式化 locale 无关声明成立）；资源面既有守卫（队列 32/画布 guardrail/上下文 200 次回收）覆盖。验证：vitest 2571、Playwright 21/21、demo 端到端、CI 绿 |
| v9.41 | 08-24 | **第四轮深度 review：GL viewport 防御性对齐 + 死导出清理（附诚实定性）**。审查新区域（共享 GL 表面/提取的 canvas 渲染器/track 侧 whenDataReady 增量）发现：①离屏 resizeCanvas 不设 gl.viewport，而上游交互路径每次 resize 都显式设置（virtual_overlay_canvas.ts:251,265）——同尺寸换 dpr 场景理论上 viewport 停留旧值致 GL 层错位。**两方向（2→1/1→2）在 Chrome 实测均不可复现**（现代实现对 buffer resize 自愈），定性为防御性对齐而非已证缺陷：已补 viewport 重置（与上游同一不变式）+ dpr 回归用例锁定契约（含双向验证记录）；②AcquiredSurfaces.surfaces/sizeChanged 死字段、getOffscreenSurfaceStats 死导出（零消费者）删除；③counter/slice/cpu_freq 的 whenDataReady 增量复核无误（memo 键序一致）。方法论记录：回归测试先证伪（移除修复双向验证），不可复现即如实降级定性。验证：vitest 2571、Playwright 20/20（+dpr 用例）、golden MATCH×2 |
| v9.40 | 08-24 | **开源级 review 修复全落地（P1-P4 全清，行为零变更）**。P1 并发竞态：渲染器模块级共享画布无重入保护（同值 width 赋值亦清空位图；postMessage 层有串行化但 API 直调裸奔）→ manager 加 in-flight promise 链串行化全部调用 + 单测（重叠检测/失败链不断）；P2 死面：drawGroupHeaderRows 死函数（-56 行）、TrackBox.isGroupHeader/expanded 恒 false 字段、`Partial<>`+`= {}` 签名放宽（改必填，postMessage 侧 unknown cast + adapter 边界校验保留）；P3 四处陈旧注释（含张冠李戴的 nodeHeight 孤儿注释）；P4：uri 解析改单遍 workspace 索引（uri→depth/node 双 map）、timedOutTracks Set 化、离屏 canvas 去死 style 赋值、freeze 窗口注释补 fonts 间隙说明。golden runner 指纹去 group 恒 false 字段（机械 rebaseline，pngHash 双场景 UNCHANGED 实证零像素漂移）。验证：vitest 2571（+2 manager 并发测试）、tsc/eslint 0、Playwright 19/19、golden MATCH×2、CI 绿。事故记录：批量 python 改脚本一次变量串写把渲染器内容写进 post_message_handler.ts，git checkout 秒级恢复+重放单点改动——批量脚本必须逐文件 assert 路径与锚点（已执行） |
...
> **人工审核门（2026-08-23 用户指令，v9.24 起）**：任何写代码、执行测试（含冒烟/基线重建）之前，方案先呈报用户审核，获确认后执行；"自主推进"模式废止。
...
| T0.3 | **M1a = PR 0**：提取 TrackTreeView 绘制序列为纯函数 | UI 主路径行为零变化（官方像素基线无 diff） | — | ✅ | commit t0.3-extract-draw-sequence | ✅ 抽取零行为变化，A/B 字节一致 SHA256 6c48de7b |
...
| T1.7 | public/timeline_image.ts + core/timeline_image_manager.ts + Trace 挂载 | 控制台 `trace.renderTimelineImage({...})` 出图；Timeline 缺席时 TIMELINE_UNAVAILABLE | T1.5 | ✅ | commit pr1-timeline-image | ✅ 注册反转模式挂载；E2E 首图 8/8 track 零 warning |
...
| T1.16 | workspace 时序竞争防护（traceInfo 就绪 ≠ defaultWorkspace 树建完；过早调用 renderTimelineImage 会报 no renderable tracks） | renderTimelineImage 在 workspace 空时等待（有限预算）或返回 WORKSPACE_NOT_READY warning，而非误报 missing | T1.7 | ✅ | 修复 + 用例 | ✅ v9.18 双层闭环（API 等 !isLoadingTrace&&tracks 30s；postMessage 层等 trace 60s） |
| T1.17 | native trace_processor 测试默认化（`trace_processor_shell -D` daemon + UI 自动协商；注意 daemon 有 preloaded trace 时 UI 会弹确认框，测试需空载 daemon） | 测试脚本默认 native：smartperfetto 14.86MB 加载 4.3s→0.9s；example 58MB 2.6s(WASM 3.3s)；脚本记录引擎证据（console "Opening trace using native accelerator"，注意勿误匹配启动期 ERR_CONNECTION_REFUSED 警告） | — | ✅ | daemon 启停 + bench 记录 | ✅ daemon 常驻 :9001 自动协商；14.86MB 4.3s→0.9s |
| T1.18 | 组件宇宙盘点与覆盖物免疫验证（用户质询"是否最多 9 个组件、弹窗是否排除"触发） | 完整矩阵：9 静态布局 + 8 类动态组件（modal/toast/tooltip/hint/spinner/popup/选中态/搜索高亮），每类标注排除机制并有证据 | — | ✅ | 组件矩阵 + 免疫实验 | ✅ 17 类组件矩阵 + 弹窗免疫逐字节实证（out/test-runs/20260823-overlay-immunity） |
...
| T1.25 | 黄金场景注册表冻结 + runner 落地 | 场景参数只改注册表（改前须用户确认）；交付图只出自 runner | T1.13,22,24 | ✅ | tools/timeline-image/run-golden.mjs | G-STD/G-DEFAULT/G-E1 参数冻结于注册表与 baselines/ |
...
| 08-23 | MCP 二次定性 (ADR-15) | 用户价值链推演："真价值在第三方 Agent 后台批量分析截图进报告；UI 内 1对1 聊天场景人正看着实时时间线，show-timeline 跳到 jank 处即可，不需要截图给人看"。与同日 review 的 R1 核验互相印证（gemini_client.ts:169-181 把 tool 结果串成纯文本进 functionResponse，模型看不到图）→ 插件内工具砍（T3.2 ❌）。MCP server 重新定性为 postMessage 的消费者之一（与 demo/渲染服务同列调用方基建），T3.6 立项：stdio + 常驻 headless Chromium + load_trace/list_tracks/render_timeline_image 工具面，零 perfetto diff。教训记录：连续两版 MCP 方案（外挂 server→插件内工具）各对一半——入口层判断对（进程外），定性错（包装成 perfetto 入口）；正确心智模型=perfetto 侧入口只有 postMessage 一个，其余全是消费者 |
